...
pub trait Runner {
    ...
    // Create a temporary snapshot, return the payload, and save the snapshot prefix
    fn create_tmp_and_next(&self, prefix_index: usize) -> (Vec<u8>, usize);
...
impl Runner for LocalRunner {
    ...
    fn create_tmp_and_next(&self, _prefix_index: usize) -> (Vec<u8>, usize) {
        (vec![], 0)
    }
...
impl Runner for NyxRunner {
    ...
    fn create_tmp_and_next(&self, prefix_index: usize) -> (Vec<u8>, usize) {
        ...
        let len = unsafe { nyx_create_tmp_and_next(data.as_mut_ptr(), &mut pos) };
        data.truncate(len);
        (data, pos)
    }
...
impl Runner for StdRunner {
    ...
    fn create_tmp_and_next(&self, prefix_index: usize) -> (Vec<u8>, usize) {
        self.runner.create_tmp_and_next(prefix_index)
    }
