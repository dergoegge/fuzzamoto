...
/** Create a temporary snapshot, fetch the payload, and save the snapshot
 *  prefix for later restores.
 */
size_t nyx_create_tmp_and_next(const uint8_t *data, size_t *prefix_index) {
  static size_t saved_prefix_index = 0;

  // Save the prefix index before taking the snapshot
  saved_prefix_index = *prefix_index;

  // Create the TMP snapshot which also captures saved_prefix_index
