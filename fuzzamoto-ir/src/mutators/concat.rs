...
impl<R: RngCore> Splicer<R> for ConcatMutator {
    fn splice(&mut self, program: &mut Program, splice_with: &Program, _rng: &mut R) -> MutatorResult {
        let mut builder = ProgramBuilder::new(program.context.clone());
        builder
            .append_program_without_threshold(program, 0usize)
            .map_err(|_| MutatorError::CreatedInvalidProgram)?;
        builder
            .append_program_without_threshold(splice_with, builder.variable_count())
            .map_err(|_| MutatorError::CreatedInvalidProgram)?;
